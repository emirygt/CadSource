...
## 🔒 Zorunlu Operasyon Kuralı (Tüm AI Agent'lar)

Bu projede **NO_DOCKER** politikası vardır.

- Docker kurulumunu **ASLA** önermeyin/çalıştırmayın (`apt/yum install docker`, `get.docker.com`, `docker compose` vb.).
- Deploy/update akışı Docker'sız olmalı: `git pull` + backend process restart (`uvicorn`/`pm2`/`systemd`) + gerekirse `nginx reload`.
- `docker-compose*.yml` ve Docker odaklı scriptler legacy kabul edilir; varsayılan operasyon kılavuzu değildir.
- Kullanıcı açıkça Docker istese bile önce bu politika hatırlatılır ve Docker'sız alternatif verilir.
...
**Çalıştığı dosyalar:**
- `ecosystem.config.js` — PM2 process yönetimi
- `nginx.conf`, `nginx/default.conf` — reverse proxy
- `start-dev.sh`, `stop-dev.sh`
- `backend/.env`, `backend/.env.example`
- `scripts/vps-deploy.sh` — legacy Docker script (varsayılan dışı)

**Kurallar:**
- Docker kurulum/çalıştırma adımı önermemeli.
- VPS güncelleme akışı Docker'sız olmalı (git pull + process restart).
- Mevcut canlı süreci bozmadan, önce çalışan dizin ve çalışan process doğrulanmalı.

**Local ortam:**
- PostgreSQL: servis olarak çalışır (Docker yok)
- Backend: port 8000
- Frontend: port 8080
