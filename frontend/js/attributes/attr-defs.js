...
const _ADF_TYPE = {
  text:    { label: 'Metin',      cls: 'adf-tb-text',    svgStroke: '#2563eb' },
  number:  { label: 'Sayı',       cls: 'adf-tb-number',  svgStroke: '#7c3aed' },
  boolean: { label: 'Evet/Hayır', cls: 'adf-tb-boolean', svgStroke: '#15803d' },
  select:  { label: 'Seçenek',    cls: 'adf-tb-select',  svgStroke: '#c2410c' },
};
...
function renderAttrDefList() {
  ...
  if (!el) return;
  if (!_attrDefs.length) {
    el.innerHTML = `<tr class="adf-empty-row"><td colspan="6">
      <div class="adf-empty-inner">
        <div class="adf-empty-icon-wrap"><svg viewBox="0 0 24 24" width="24" height="24" fill="none" stroke="#94a3b8" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round"><rect x="3" y="3" width="18" height="4" rx="1"/><rect x="3" y="10" width="18" height="4" rx="1"/><rect x="3" y="17" width="18" height="4" rx="1"/></svg></div>
  ...
  el.innerHTML = _attrDefs.map(d => {
    const tp = _ADF_TYPE[d.data_type] || _ADF_TYPE.text;
    const pills = (d.options || []).map(o => `<span class="adf-option-pill">${escHtml(o)}</span>`).join('');
    return `<tr>
      <td><span class="adf-name-cell">${escHtml(d.name)}</span></td>
      <td><span class="adf-type-badge ${tp.cls}">${tp.label}</span></td>
      <td>${d.unit ? `<span class="adf-unit-tag">${escHtml(d.unit)}</span>` : '<span style="color:#cbd5e1">—</span>'}</td>
      <td style="text-align:center">${d.required ? '<span class="adf-required-dot"></span>' : '<span style="color:#e2e8f0">○</span>'}</td>
      <td>${pills || '<span style="color:#cbd5e1;font-size:12px">—</span>'}</td>
      <td>
        <button class="adf-del-btn" onclick="deleteAttrDef(${d.id})" title="Sil">
          <svg viewBox="0 0 24 24" width="14" height="14" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><polyline points="3 6 5 6 21 6"/><path d="M19 6l-1 14a2 2 0 01-2 2H8a2 2 0 01-2-2L5 6"/><path d="M10 11v6M14 11v6"/><path d="M9 6V4h6v2"/></svg>
        </button>
      </td>
    </tr>`;
  }).join('');
...
function adfOpenModal() {
  document.getElementById('adfOverlay').classList.add('open');
  setTimeout(() => document.getElementById('attrNameInput')?.focus(), 60);
}

function adfCloseModal() {
  document.getElementById('adfOverlay').classList.remove('open');
  document.getElementById('attrAddMsg').style.display = 'none';
}

function adfTypeChange() {
  const type = document.querySelector('input[name="attrType"]:checked')?.value;
  const row = document.getElementById('adfOptionsRow');
  if (row) row.style.display = type === 'select' ? '' : 'none';
}

document.addEventListener('change', e => {
  if (e.target.name === 'attrType') adfTypeChange();
});

async function addAttrDef() {
  const name = document.getElementById('attrNameInput').value.trim();
  const data_type = document.querySelector('input[name="attrType"]:checked')?.value || 'text';
  ...
  if (!name) { msg.style.display='block'; msg.textContent='Alan adı boş olamaz'; return; }
  ...
  if (!r.ok) { const e = await r.json(); msg.style.display='block'; msg.textContent=e.detail||'Hata'; return; }
  ...
  const firstRadio = document.querySelector('input[name="attrType"][value="text"]');
